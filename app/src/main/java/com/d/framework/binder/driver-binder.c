			      ...
			      binder_size_t *consumed, int non_block)
{
	...
	if (*consumed == 0) {
	    // *consumed == 0 写入一个值BR_NOOP到参数ptr指向的缓冲区中去，即用户传进来的bwr.read_buffer缓冲区
...
retry:
    ...
    // thread->transaction_stack == NULL，并且thread->todo列表也是空的，这表示当前线程没有事务需要处理，
    // 于是wait_for_proc_work为true，表示要去查看proc是否有未处理的事务
	...
	if (wait_for_proc_work)
		proc->ready_threads--;
	// thread的状态为BINDER_LOOPER_STATE_WAITING，表示线程处于等待状态
	thread->looper &= ~BINDER_LOOPER_STATE_WAITING;
...
static int binder_ioctl_set_ctx_mgr(struct file *filp)
{
    ...
    // binder_context_mgr_node 是一个 binder_node 标示 Service Manager实体
	...
	if (ret < 0)
		goto out;
	// binder_context_mgr_uid 是Service Manager守护进程的Uid
...
static int binder_mmap(struct file *filp, struct vm_area_struct *vma)
{
    ...
    // vm_struct 是给内核使用, vm_area_struct是进程空间使用的
    // 地址范围 3G ????
	...
	preempt_disable();
	// proc->buffer, proc->buffer + PAGE_SIZE 分配物理页面
